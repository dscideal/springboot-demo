...
public class AutoGenerator {
    ...
    private Map<String, VelocityContext> analyzeData(ConfigBuilder config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            ctx.put("entity", tableInfo.getEntityName());// 实体名
            ctx.put("entityField", tableInfo.getEntityNameField());// 实体首字母小写
            ctx.put("serviceField", tableInfo.getServiceField());// 服务名 首字母小写
            ctx.put("serviceFieldU", tableInfo.getServiceFieldU());// 服务名 首字母小写
    ...
    private void batchOutput(String entityName, VelocityContext context) {
        try {
            ...
            String entityFile = String.format((pathInfo.get(ConstVal.ENTITY_PATH) + File.separator + "%s" + this.suffixJavaOrKt()), entityName+"Model");
            ...
            vmToFile(context, "templates/msController.java.vm", "d:/develop/code/msContoller/"+entityName+"Contoller.java");
            vmToFile(context, "templates/msService.java.vm", "d:/develop/code/msService/"+entityName+" Service.java");
